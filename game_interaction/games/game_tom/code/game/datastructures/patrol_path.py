...
class PatrolPath:
    ...
    def __init__(self, patrol_type="circular"):
        ...
        """
        # DONE: Initialize head, tail, current to None
        self.head = None
        self.tail = None
        self.current = None
        # DONE: Store patrol_type
        self.patrol_type = patrol_type
        # DONE: Initialize size to 0
        self.size = 0
        # DONE: Initialize direction to 1 (1 = forward, -1 = backward for back_and_forth)
        self.direction = 1
    ...
    def add_waypoint(self, x, y, wait_time=0):
        ...
        """
        # DONE: Create a new Waypoint node
        self.new_waypoint = Waypoint(x, y, wait_time)
        # DONE: If the list is empty, set head, tail, and current to the new node
        if (self.size == 0):
            self.head = self.new_waypoint
            self.tail = self.new_waypoint
            self.current = self.new_waypoint
            
        # DONE: Otherwise, link the new node after the current tail
        else:
            self.tail.next= self.new_waypoint
            # DONE: For "back_and_forth" and "circular", set prev pointer on the new node
            if((self.patrol_type == "back_and_forth") or (self.patrol_type == "circular")):
                self.new_waypoint.prev = self.tail
            # DONE: Update tail to the new node
            self.tail = self.new_waypoint
            # DONE: For "circular", close the loop (tail.next = head, head.prev = tail)
    ...
    def get_next_waypoint(self):
        ...
        """
        # DONE: If empty or current is None, return None
        if(self.current == None):
            return None
            
        # DONE: Save current as result to return
        result = self.current
        
        # DONE: For "one_way": advance current to current.next (becomes None at end)
        if(self.patrol_type == "one_way"):
            self.current = self.current.next
                
        # DONE: For "circular": advance current to current.next (wraps around)
        elif(self.patrol_type == "circular"):
            # if self.current == self.tail, point to head
        ...
        # DONE: For "back_and_forth": advance forward or backward based on direction,
        #       reversing direction when hitting the end or start
        elif(self.patrol_type == "back_and_forth"):

            if(self.direction == 1):
                # if currently at end of list goint forward, reverse
                if(self.current == self.tail):
                    self.direction = -1
                ...
                # if currently at front of list going backward, reverse
