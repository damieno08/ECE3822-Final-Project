...
class ArcadeClient:
    def __init__(self):
        ...
        # Centralized Metadata to fix image loading issues
        self.game_metadata = {
    ...
    def show_game_search(self):
        ...
        # Map names back to our metadata keys (0-4)
        rec_data = []
        for name, _ in recs:
            # Match metadata name to recommendation name
            found = next((v for v in self.game_metadata.values() if v[0].startswith(name.upper())), None)
            if found: rec_data.append(found)
        if not rec_data: rec_data = list(self.game_metadata.values())[:3] # Fallback
    ...
    def show_lookup(self):
        ...
        self.search_entry = tk.Entry(search_f, bg="#111", fg=self.colors["fg"], insertbackground=self.colors["fg"], font=("Courier", 12))
    ...
    def display_user_profile(self, user_obj):
        # Profile view (re-uses result display window)
        self.display.config(state='normal'); self.display.delete('1.0', tk.END)
        ...
        self.display.insert(tk.END, "SESSION_HISTORY:\n")
        for i in range(user_obj.get_total_games()):
            self.display.insert(tk.END, f" • {user_obj.get_history('game', i)}\n")
        self.display.config(state='disabled')
    ...
    def run_game(self, idx):
        try:
            ...
            self.s.sendall(f"SAVE_SESSION|{self.current_user.name}|{score}|{duration}|{idx}".encode())
            self.root.after(0, lambda: self.show_play(idx))
        except Exception as e:
            messagebox.showerror("CORE_ERROR", str(e))
