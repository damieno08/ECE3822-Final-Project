...
def run():
    ...
    # ── get_history: single peek on a history of depth N ────────────────────
    def setup_peek(n):
        h   = History()
        ses = _make_session()
        for _ in range(n):
    ...
    # ── pop: single pop on a history of depth N ──────────────────────────────
    def setup_pop(n):
        h   = History()
        ses = _make_session()
        for _ in range(n):
    ...
    # ── Play_history.get_total_time: O(n) scan ───────────────────────────────
    ...
    # Graph 1: O(n) total-cost operations (push N, scan N)
    save_plot(
    ...
    # Graph 2: single-call latency (get_history, pop) -- should be flat O(1)
    save_plot(
