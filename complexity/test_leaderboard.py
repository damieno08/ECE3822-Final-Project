...
def _build(n, seed=42):
    """Return a Leaderboard pre-populated with n unique users."""
    random.seed(seed)
    lb = Leaderboard()
    for i in range(n):
...
def run():
    ...
    # Graph 1: build cost (total N insertions)
    save_plot(
        "leaderboard_build.png",
        "Leaderboard -- build cost (N insertions total)",
        [
            {"label": "add_score  N total",  "rows": rows_add, "complexity": "O(n log n)"},
        ],
    )

    # Graph 2: single-query latency vs. leaderboard size
    save_plot(
